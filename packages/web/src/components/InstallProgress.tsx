...
export function InstallProgress({ serviceId, serviceName, onDone }: InstallProgressProps) {
  ...
  const serviceSteps = Object.entries(status?.steps ?? {})
    ...
    .map(([key, stepStatus]) => ({ key, label: formatLabel(key, serviceId), status: stepStatus }));
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold mb-2">
          {isSuccess ? `${serviceName} installed` : isDone ? "Installation failed" : `Installing ${serviceName}...`}
        </h2>
