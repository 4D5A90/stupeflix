...
export function serviceUrl(raw: string, containers: Iterable<string>): string {
	...
	// `hostname` and not `host`: it drops the port, and it strips the brackets an
	// IPv6 literal carries. It also ignores userinfo, which is how
	// `http://127.0.0.1@evil.com` used to read as loopback.
