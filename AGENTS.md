...
- `category`: **libre**. Une catégorie que le front ne connaît pas ne casse rien —
  `categoryLabel()` dérive son titre de l'id (`usenetClient` → « Usenet Client »),
  elle se range en fin de liste et prend l'icône par défaut, comme un service sans
  glyphe prend `_default.svg`. Ce n'est **pas** un substitut au fait de la nommer :
  une catégorie qui compte reçoit son libellé, son icône et sa place, et ce sont
  des choix éditoriaux qu'aucune dérivation ne fait à ta place. Adapter le produit
  quand un nouveau *type* d'outil entre fait partie du produit — les catégories
  déclaratives ont été écartées pour ça. Les libellés sont **volontairement en
  double** (`steps/ServicesStep.tsx` titre une section « Media Server »,
  `dashboard/categories.ts` a la place pour « Media » sous un nom de service)
...
`KNOWN_CATEGORIES` is a gate of a different kind, and only on what this repo
**ships**: an unknown category renders fine for a dropped-in template, so it is
there to catch `mediaServr` before it quietly opens a category of one.
